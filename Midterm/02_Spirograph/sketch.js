...
var fund = 5; // the speed of the central sine
var ratio = 0.0000006666; // what multiplier for speed is each additional sine?
var alpha = 5000; // how opaque is the tracing system
...
function draw()
{
 ...
 var x = mouseX;
  ...
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    ...
    // setup for tracing
    if (trace) {
      stroke(random(255), random(255), 255*(float(i)/sines.length), alpha); // blue
      fill(random(255), random(255), 255, alpha/2); // also, um, blue
