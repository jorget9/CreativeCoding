...
function draw() {
  ...
  image(img[whichimage],  random(255),  random(255), width, height); // draw the new source
}
...
function randomize()
{
  ...
  for (var i = 0; i < img[whichimage].width; i++) {
    for (var j = 0; j < img[whichimage].height; j++) {
      ...
      img[whichimage].set(i, j, random(255), random(255));
      img[1-whichimage].set(i, j, random(255), random(255));
    }
  }
